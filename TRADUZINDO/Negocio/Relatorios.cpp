...
void Relatorios::tabela(ListaPaciente *lista,int cont){

    int intervalo,idade,intervaloma;
    float porcentagem,c1= 0,porcentagemtotal;
    cout << "Digite o intervalo entre as idades: ";
    cin >> intervalo;
    cout  << endl << endl;
    cout << "Idade\t\t" << "Frequencia\t" << "Porcentagem" << endl << endl;
    for (int i = 0; i < 5; i++){
        intervalo = intervalo;
        intervaloma = intervalo + 5;
        cout << intervalo  << " ~ " << intervaloma << "\t\t"  ;
        lista->atual = lista->primeiro;
        while(lista->atual != NULL){
            if(lista->atual->item->getIdade() > intervalo && lista->atual->item->getIdade() < intervaloma){
                c1++;
            }
            lista->atual = lista->atual->proximo;
        }
        porcentagem = (c1/cont) * 100;
        porcentagemtotal += (c1/cont) * 100;
        cout << c1 << "\t\t" << porcentagem << "%" <<endl;
        intervalo = intervaloma;
        c1 = 0;
    }
    cout << endl << porcentagemtotal << "%  dos pacientes cadastrados estão incluidos na faixa selecionada." << endl << endl;
}
